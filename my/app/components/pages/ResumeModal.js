...
//         // Apply professional filter
//         filterResumeData(data, role, interviewType);
...
export default function ResumeModal({ isOpen, onClose }) {
  const router = useRouter();
  const { setResumeData, setQuestions, setResumeSummary, setInterviewType } = useAppContext();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data) {
        ...
        // FIX: use correct returned structure
        const { cleanText, questions: generatedQuestions } = filterResumeData(data, role, interviewType);
