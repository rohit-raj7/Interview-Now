...
# HR templates split into two categories
HR_QUESTION_GENERAL = [
    "Tell me about yourself.",
    "How would you describe yourself in one sentence?",
    "What motivates you to work on yourself?",
...
# Entity types to consider
KEY_LABELS = ['ORG', 'PRODUCT', 'SKILL', 'LANGUAGE', 'EVENT']
...
def extract_keywords_and_questions(text):
    ...
    # HR questions → Mix of general + keyword-based
    hr_qs = []
    hr_qs.extend(random.sample(HR_QUESTION_GENERAL, min(5, len(HR_QUESTION_GENERAL))))

    for k in hr_keywords:
        selected_qs = random.sample(HR_QUESTION_KEYWORD_BASED, min(3, len(HR_QUESTION_KEYWORD_BASED)))
        hr_qs.extend(q.format(k) for q in selected_qs)
